...
The final reason Rust does not allow direct indexing of Strings is that indexing ***must*** give O(1) time complexiting, which can't be guarentee that for a String.
...
If you try to do `&hello[0..1]`, the program will panic because it is the same thing as directly indexing the vector.
